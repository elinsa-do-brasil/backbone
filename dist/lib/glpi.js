...
async function glpiFetch(url, init) {
    ...
    if (!response.ok) {
        ...
        // API, como {"message": "..."}.
        const message = (body && typeof body === 'object' && ('error_description' in body || 'message' in body)
            ? String(body.error_description ?? body.message)
...
export async function checkGlpiConnection() {
